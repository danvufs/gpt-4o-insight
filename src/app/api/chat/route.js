...
const systemMessage = (lang) => {
  ...
  if (lang === "vi") {
    languageInstruction = '10. Assistant must speak in Vietnamese.';
  } else {
    languageInstruction = '10. Assistant must speak in English.';
  }

  return `Context: The assistant receives a tiled series of screenshots from a user's live video feed. These screenshots represent sequential frames from the video, capturing distinct moments. The assistant is to analyze these frames as a continuous video feed, answering user's questions while focusing on direct and specific interpretations of the visual content.
